vkGetAdStatistics <- function(account_id = NULL,
  ...
  if(!is.null(dataRaw$error)){
    ...
    result  <- rbind(result,
                     data.frame(id                  = ifelse(is.null(dataRaw$response[[i]]$id), NA,dataRaw$response[[i]]$id),
                                ...
                                spent               = ifelse(is.null(dataRaw$response[[i]]$stats[[dt]]$spent), 0,dataRaw$response[[i]]$stats[[dt]]$spent),
                                impressions         = ifelse(is.null(dataRaw$response[[i]]$stats[[dt]]$impressions), 0,dataRaw$response[[i]]$stats[[dt]]$impressions),
                                clicks              = ifelse(is.null(dataRaw$response[[i]]$stats[[dt]]$clicks), 0,dataRaw$response[[i]]$stats[[dt]]$clicks),
                                reach               = ifelse(is.null(dataRaw$response[[i]]$stats[[dt]]$reach), 0,dataRaw$response[[i]]$stats[[dt]]$reach),
                                ...
                                join_rate           = ifelse(is.null(dataRaw$response[[i]]$stats[[dt]]$join_rate ), 0,dataRaw$response[[i]]$stats[[dt]]$join_rate),
                                stringsAsFactors = F))}
    }
   if(period == "month"){
   for(dt in 1:length(dataRaw$response[[i]]$stats)){
  ...
  if (length(result$clicks)>0) result$clicks <- as.integer(result$clicks)
  if (length(result$join_rate)>0) result$join_rate <- as.integer(result$join_rate)
  return(result)
}
